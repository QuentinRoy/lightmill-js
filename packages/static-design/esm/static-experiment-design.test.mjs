...
describe('StaticExperimentDesign#getAvailableRuns', () => {
  it('returns the list of un-started available runs from the list of started runs', async () => {
    await expect(
      design.getAvailableRuns(['mock-run-2', 'mock-run-3'])
    ).resolves.toEqual(['mock-run-1', 'mock-run-4']);
  });
  it('returns an empty list if there is no available runs', async () => {
    await expect(
...
describe('StaticExperimentDesign#startRun', () => {
  it('returns the run iterator for the specified run', async () => {
    await expect(design.startRun('mock-run-3', 'options')).resolves.toBe(
      'mock-iterator-mock-run-3'
    );
    await expect(StaticRunDesign.mock.calls).toEqual([
      [{ id: 'mock-run-3', blocks: [{ trials: [{ data: '3' }] }] }, 'options'],
    ]);
  });
  it('fails if the run design does not exists', async () => {
    await expect(design.startRun('unknown-run-id')).rejects.toThrow();
  });
});
