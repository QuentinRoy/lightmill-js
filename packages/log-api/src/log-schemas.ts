...
const LogNotFoundErrorResponse = getErrorDocumentSchema(
  ...
  '/': {
    get: {
      request: {
        ...
        headers: z.looseObject({
          accept: z.string().optional() as z.ZodType<
            // Little trick for TypeScript auto-completion to suggest the media
            // type or csv.
            (string & {}) | 'text/csv' | typeof mediaType
          >,
        }),
      },
      responses: {
        200: {
          description: `List of logs. Format is CSV by default, or JSON if the Accept header is set to ${mediaType}.`,
          content: {
    ...
    post: {
      ...
      request: {
        body: {
          required: true,
          content: { [mediaType]: { schema: LogPostRequest } },
        },
      },
      responses: {
        201: {
