...
const SessionGetResponse = getDataDocumentSchema({
  data: SessionResource,
  includes: z.union([RunResource, LogResource, ExperimentResource]),
...
// Error schemas
// -----------------------------------------------------------------------------
const SessionNotFoundErrorResponse = getErrorDocumentSchema(
  getErrorSchema({ code: 'SESSION_NOT_FOUND', statusCode: 404 }),
).openapi('SessionNotFoundErrorResponse');
const SessionExistsErrorResponse = getErrorDocumentSchema(
  getErrorSchema({ code: 'SESSION_EXISTS', statusCode: 409 }),
).openapi('SessionExistsErrorResponse');
const InvalidCredentialErrorResponse = getErrorDocumentSchema(
  getErrorSchema({
  ...
  '/': {
    post: {
      ...
      request: {
        body: {
          required: true,
          content: { [mediaType]: { schema: SessionPostRequest } },
        },
